...
const Reviewpending = () => {
  ...
  const getCourse = () => {
    axios
      .get(`${process.env.REACT_APP_API_URL}/api/admin/course/review-pending`, {
        headers: {
  ...
  const handleHold = (_id) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_URL}/api/admin/course/review-pending/edit/${_id}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${uToken}`,
  ...
  const handleApprove = (_id) => {
    ...
    axios
      .put(
        `${process.env.REACT_APP_API_URL}/api/admin/course/review-pending/approved/edit/${_id}`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${uToken}`,
    ...
    <div className="card container " style={{ marginBottom: 150 }}>
      <div className="card-header border-0 pt-5">
        <h3 className="card-title align-items-start flex-column">
          <span className="card-label fw-bolder fs-3 mb-1">ReviewPending</span>
        </h3>
      </div>
